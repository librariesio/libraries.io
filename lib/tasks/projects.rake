...
namespace :projects do
  ...
  task check_status: :environment do
    exit if ENV["READ_ONLY"].present?
    %w[npm rubygems packagist nuget wordpress cpan clojars cocoapods
  ...
  task check_removed_status: :environment do
    exit if ENV["READ_ONLY"].present?
    %w[npm rubygems packagist wordpress cpan clojars cocoapods
       hackage cran atom sublime pub elm dub].each do |platform|
  ...
  task download_missing: :environment do
    exit if ENV["READ_ONLY"].present?
    %w[Alcatraz Atom Bower Cargo Clojars CocoaPods CRAN
