...
namespace :research do
  ...
  task civic_tech: :environment do
    ...
    project_counts = projects.each_with_object (Hash.new(0)) { |el, counts| counts[el] += 1; }

    project_counts.select { |_k, v| v > 1 }.sort_by { |_k, v| -v }.each { |k, v| puts "#{k} (#{v})" }; nil
  end
  ...
  task civic_tech_deps: :environment do
    ...
    project_counts.select { |_k, v| v > 0 }.sort_by { |_k, v| -v }.each { |k, v| project = Project.find(k); puts "#{project.platform}/#{project.name} (#{v})" }; nil

    json = project_counts.select { |_k, v| v > 0 }.sort_by { |_k, v| -v }.map do |k, v|
    ...
    unseen = unique_cii_deps.select do |project|
      project.stars < 100
    ...
    cii_projects.group_by(&:normalize_licenses).sort_by { |_k, v| -v.length }.each { |k, v| puts "#{k.join('/')} (#{v.length})" }; nil

    # civic tech deps license breakdown

    dependencies.group_by(&:normalize_licenses).sort_by { |_k, v| -v.length }.each { |k, v| puts "#{k.join('/')} (#{v.length})" }; nil
  ...
  task top_cii_contributors: :environment do
    ...
    top_1percent = contributor_ids.select { |_k, v| v > 499 }.map { |k, v| [RepositoryUser.find(k), v] }
  ...
  task backported_projects: :environment do
    ...
    backporting = projects.reject do |project|
      releases = project.stable_releases.select(&:greater_than_1?).uniq(&:published_at)
  ...
  task version_breakdowns: :environment do
    ...
    latest_versions = Project.platform(platform).order("(dependent_repos_count) DESC NULLS LAST").limit(10000).pluck(:latest_release_number).compact.map { |v| v.split(".").first.to_i }.sort.each_with_object(Hash.new(0)) { |e, h| h[e] += 1; }
  end
end
