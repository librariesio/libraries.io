...
describe "Api::SubscriptionsController" do
  ...
  def subscription_json_response(subscription)
    subscription.as_json(
      only: [:include_prerelease, :created_at, :updated_at],
      include: {
        project: {
          only: Project::API_FIELDS,
          methods: [:package_manager_url, :stars, :forks, :keywords, :latest_stable_release, :latest_download_url, :scm_license],
          include: {
            versions: {
  ...
  describe "GET /api/subscriptions", type: :request do
    it "renders successfully" do
      ...
      expect(response.body).to be_json_eql [subscription_json_response(subscription)].to_json
    end
  end
  ...
  describe "PUT /api/subscriptions/:platform/:name", type: :request do
    it "renders successfully" do
      ...
      expect(response).to have_http_status(:success)
      expect(response.content_type).to eq('application/json')
      expect(response.body).to be_json_eql subscription_json_response(subscription.reload).to_json
    end
  end
