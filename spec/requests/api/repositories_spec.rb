...
describe "Api::RepositoriesController" do
  ...
  describe "GET /api/github/:owner/:name/dependencies", type: :request do
    it "renders successfully" do
      ...
      expect(response).to have_http_status(:success)
      expect(response.content_type).to eq('application/json')
      expect(json.to_json).to be_json_eql @repo.as_json({except: [:id, :repository_organisation_id, :repository_user_id], methods: [:github_contributions_count, :github_id]}).merge(dependencies: []).to_json
    end
  end
  ...
  describe "GET /api/github/:owner/:name", type: :request do
    it "renders successfully" do
      ...
      expect(response).to have_http_status(:success)
      expect(response.content_type).to eq('application/json')
      expect(json.to_json).to be_json_eql @repo.to_json({except: [:id, :repository_organisation_id, :repository_user_id], methods: [:github_contributions_count, :github_id]})
    end
  end
end
