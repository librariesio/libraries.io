...
describe "HooksController" do
  describe "GET /priority/:platform/:name/:priority", type: :request do
    ...
    it "403s without api key" do
      get "/api/priority/rubygems/rails/low"
      expect(response).to have_http_status(:forbidden)
    end

    it "403s without external api key" do
      get "/api/priority/rubygems/rails/low?api_key=#{api_key_external.access_token}"
      expect(response).to have_http_status(:forbidden)
    end

    it "404s with internal api key and non-existent project" do
      get "/api/priority/fake/rails/low?api_key=#{api_key_internal.access_token}"
      expect(response).to have_http_status(:not_found)
    end

    it "400s with bad priority" do
      get "/api/priority/rubygems/rails/fake?api_key=#{api_key_internal.access_token}"
      expect(response).to have_http_status(:bad_request)
    end

    it "successfully creates a priority object with a valid request" do
      get "/api/priority/rubygems/rails/low?api_key=#{api_key_internal.access_token}"
