...
describe MaintenanceStats::Stats::Github::ReleaseStats do
  ...
  context "with a valid repository" do
    ...
    it "should have repository release stats" do
      ...
      expect(results[:last_week_releases]).to eql 14
      expect(results[:last_month_releases]).to eql 14
      expect(results[:last_two_month_releases]).to eql 14
      expect(results[:last_year_releases]).to eql 18

      expect(results[:last_release_date]).to eql "2022-12-08T18:47:48Z"
    end
