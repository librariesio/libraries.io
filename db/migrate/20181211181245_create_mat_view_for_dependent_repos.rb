class CreateMatViewForDependentRepos < ActiveRecord::Migration[5.1]
  ...
  def up
    ...
    execute 'create materialized view project_dependent_repositories as
            ...
            from (SELECT "repositories".* , repository_dependencies.project_id FROM "repositories"
            INNER JOIN "repository_dependencies" ON "repositories"."id" = "repository_dependencies"."repository_id"
            WHERE "repositories"."private" = false GROUP BY repositories.id, repository_dependencies.project_id)
            as t1 inner join projects on t1.project_id = projects.id with no data;'
  ...
  def down
    execute 'drop materialized view project_dependent_repositories;'
  end
end
