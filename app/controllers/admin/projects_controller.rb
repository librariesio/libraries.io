class Admin::ProjectsController < Admin::ApplicationController
  ...
  def deprecated
    ...
    @projects = @search.records.where("status IS ? OR status = ''", nil).order(:rank).paginate(page: params[:page])
    if @projects.empty?
      repo_ids = GithubRepository.with_projects.where("github_repositories.description ilike '%deprecated%'").pluck(:id)
      @projects = Project.where("status IS ? OR status = ''", nil).where(github_repository_id: repo_ids).order(:rank).paginate(page: params[:page])
    end
  end
