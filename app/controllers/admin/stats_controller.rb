class Admin::StatsController < Admin::ApplicationController
  ...
  def index
    ...
    @recent_subscriptions = Subscription.where(repository_subscription_id: nil).order('created_at DESC').limit(20)
    @recent_watches = RepositorySubscription.order('created_at DESC').limit(20)
