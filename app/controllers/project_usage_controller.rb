class ProjectUsageController < ApplicationController
  ...
  def show
    @all_counts = @project.repository_dependencies.where('github_repositories.fork = ?', false).joins(manifest: :github_repository).distinct('manifests.github_repository_id').group('repository_dependencies.requirements').count.select{|k,v| k.present? }
