class ProjectUsageController < ApplicationController
  ...
  def show
    @all_counts = @project.repository_dependencies.where('github_repositories.fork = ?', false).joins(manifest: :repository).distinct('manifests.github_repository_id').group('repository_dependencies.requirements').count.select{|k,v| k.present? }
    @total = @all_counts.sum{|k,v| v }
    if @all_counts.any?
      @kinds = @project.repository_dependencies.where('github_repositories.fork = ?', false).joins(manifest: :repository).distinct('manifests.github_repository_id').group('repository_dependencies.kind').count
