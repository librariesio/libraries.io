...
class RepositoryUsersController < ApplicationController
  ...
  def show
    ...
    @contributions = if @user.org?
                       []
                     else
                       find_contributions.limit(6)
                     end
  end
  ...
  def dependencies
    ...
    @platforms = orginal_scope.pluck(:platform).each_with_object(Hash.new(0)) { |v, h| h[v] += 1;  }.sort_by { |_k, v| v }.reverse.first(20)
  end
