...
module RepositoryIssue
  class Gitlab < Base
    ...
    def self.create_from_hash(name_with_owner, issue_hash, _token = nil)
      ...
      user = begin
               RepositoryUser.where(host_type: "GitLab").find_by_uuid(issue_hash[:author][:id]) || RepositoryOwner::GitLab.download_user_from_host("GitLab", issue_hash[:author][:username])
