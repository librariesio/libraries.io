class GitterNotifications
  class << self
    def new_project(project_name, platform)
      post_to_gitter "[#{platform}] **[#{project_name}](http://libraries.io/#{platform}/#{project_name})** has been added"
    end

    def new_version(project_name, platform, version)
      post_to_gitter("[#{platform}] **[#{project_name}](http://libraries.io/#{platform}/#{project_name}/#{version})** #{version} has been added")
    end
